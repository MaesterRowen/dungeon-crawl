...
@export var sound_effects: SoundEffectList

func _ready() -> void:
	for sound_effect: SoundEffect in sound_effects.data:
...
func create_2d_sound_at_location( location: Vector2, type: SoundEffect.SOUND_EFFECT_TYPE) -> void:
	if sound_effect_dict.has(type):
		...
		if sound_effect.has_open_limit():
			...
			new_2D_audio.bus = sound_effect.audio_bus
...
func create_sound(type: SoundEffect.SOUND_EFFECT_TYPE) -> void:
	if sound_effect_dict.has(type):
		var sound_effect: SoundEffect = sound_effect_dict[type]
		if sound_effect.has_open_limit():
			sound_effect.change_audio_count(1)
			...
			new_audio.bus = sound_effect.audio_bus
